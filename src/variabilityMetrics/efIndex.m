...
%efIndex function that computes the excursion frequency (EF) index, i.e.,
%the number of excursion > 75 (ignores nan values).
%
%Input:
%   - data: a timetable with column `Time` and `glucose` containing the 
%   glucose data to analyze (in mg/dl). 
%Output:
%   - efIndex: the excursion frequency (EF) index
%
%Preconditions:
...
% ------------------------------------------------------------------------
    ...
    %Check preconditions 
    if(~istimetable(data))
        error('efIndex: data must be a timetable.');
    end
    if(var(seconds(diff(data.Time))) > 0 || isnan(var(seconds(diff(data.Time)))))
        error('efIndex: data must have a homogeneous time grid.')
    end
    if(~any(strcmp(fieldnames(data),'Time')))
        error('efIndex: data must have a column named `Time`.')
    end
    if(~any(strcmp(fieldnames(data),'glucose')))
        error('efIndex: data must have a column named `glucose`.')
    end
