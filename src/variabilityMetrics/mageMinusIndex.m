...
%mageMinusIndex function that computes the mean amplitude of negative 
...
%Output:
%   - mageMinusIndex: the mean amplitude of positive glycemic excursion (MAGE-) index
%
%Preconditions:
...
% ------------------------------------------------------------------------
    ...
    %Check preconditions 
    if(~istimetable(data))
        error('mageMinusIndex: data must be a timetable.');
    end
    if(var(seconds(diff(data.Time))) > 0 || isnan(var(seconds(diff(data.Time)))))
        error('mageMinusIndex: data must have a homogeneous time grid.')
    end
    if(~any(strcmp(fieldnames(data),'Time')))
        error('mageMinusIndex: data must have a column named `Time`.')
    end
    if(~any(strcmp(fieldnames(data),'glucose')))
        error('mageMinusIndex: data must have a column named `glucose`.')
    end
    ...
    for d = 1:nDays
        ...
        if(n > 3)
            ...
            mageDayMinus(d) = nanmean(excursions(excursions<0));
